...
export class AuthorService {
  ...
  protected convertDateFromClient(author: IAuthor): IAuthor {
    const copy: IAuthor = Object.assign({}, author, {
      endDate: author.endDate && author.endDate.isValid() ? author.endDate.format(DATE_FORMAT) : undefined,
    });
    return copy;
  }
  ...
  protected convertDateArrayFromServer(res: EntityArrayResponseType): EntityArrayResponseType {
    if (res.body) {
      res.body.forEach((author: IAuthor) => {
        author.endDate = author.endDate ? moment(author.endDate) : undefined;
      });
    }
    return res;
  }
